...
const FindDoctor: React.FC = () => {
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6">
            <Reveal>
                <h1 className="text-2xl font-bold animate-slide-in-up">Book an Appointment</h1>
            </Reveal>
            <Card className="animate-zoom-in">
                <CardHeader>
                    <Reveal>
                        <CardTitle className="animate-slide-in-up">Find a Specialist</CardTitle>
                    </Reveal>
                    <Reveal delay={0.06}>
                        <CardDescription className="animate-fade-in-up">Select a specialty to find a doctor that fits your needs.</CardDescription>
                    </Reveal>
                </CardHeader>
                <CardContent>
                    <Reveal>
            ...
            <div className="space-y-4">
                <Reveal>
                <h2 className="text-xl font-semibold animate-slide-in-up">
                    {selectedSpecialty ? `Showing ${selectedSpecialty}s` : "Available Doctors"}
                </h2>
                </Reveal>
                {filteredDoctors.length > 0 ? (
                    filteredDoctors.map((doctor, idx) => (
                        <motion.div key={doctor.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.5, delay: 0.05 * idx }}>
                        <Card className="hover:shadow-lg transition-shadow">
                            <CardContent className="p-4 flex flex-col md:flex-row items-start md:items-center gap-4">
                                ...
                                <Button onClick={() => handleBookAppointment(doctor.name)} className="mt-2 md:mt-0 bg-gradient-accent text-white hover:opacity-90">
